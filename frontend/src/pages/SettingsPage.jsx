...
function SettingsPage() {
  ...
  return <div className="dashboard-page settings-page">
    ...
    <header className="topbar settings-topbar"><strong>Onboarding Workspace</strong><nav><a href="#provisioning">Provisioning</a></nav><button aria-label="Notifications">♢</button><div className="avatar avatar--small">JD</div></header>
    ...
    <main className="settings-main"><header className="settings-heading"><h1>Organization Settings</h1><p>Manage your enterprise environment, security policies, and automation triggers.</p></header>
      <div className="settings-layout"><aside className="settings-tabs">{settingTabs.map((tab) => <button className={activeTab === tab ? 'active' : ''} onClick={() => selectTab(tab)} key={tab}><span>{tab}</span><b>›</b></button>)}<article><strong>ϟ AI Optimizer</strong><p>Enable Smart-Provision to automatically suggest role-based permissions based on employee titles.</p></article></aside>
