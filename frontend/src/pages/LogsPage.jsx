...
function LogsPage() {
  ...
  return <div className="dashboard-page logs-page">
    ...
    <header className="topbar logs-topbar"><strong>Onboarding Workspace</strong><div className="logs-search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search logs, employees, or events..." /></div><nav><a href="#provisioning">Provisioning</a></nav><button>♢</button></header>

    <main className="logs-main">
      <aside className="logs-filters"><header><h2>Filters</h2><button onClick={resetFilters}>Reset</button></header><section><h3>Status</h3>{['Success', 'Warning', 'Critical'].map((status) => <label key={status}><input type="checkbox" checked={statuses.includes(status)} onChange={() => toggleStatus(status)} />{status}</label>)}</section><section><h3>Application</h3><select value={application} onChange={(event) => setApplication(event.target.value)}><option>All Applications</option><option>Slack</option><option>Jira Cloud</option><option>AWS Console</option><option>GitHub Enterprise</option></select></section><section><h3>Admin</h3><input placeholder="Search admins..." /></section><section><h3>Date Range</h3><input type="date" /><input type="date" /></section></aside>
      <section className="logs-content"><header className="logs-heading"><div><h1>Event Logs</h1><p>Real-time monitoring of all provisioning and identity actions.</p></div><div className="logs-actions"><div><button className={view === 'Table' ? 'active' : ''} onClick={() => setView('Table')}>☷ Table</button><button className={view === 'Timeline' ? 'active' : ''} onClick={() => setView('Timeline')}>⌁ Timeline</button></div><button>⇩ Export Data</button></div></header>
        <aside className="logs-insight"><span>✦</span><div><strong>AI Detection: Anomaly in GitHub Access</strong><p>We noticed 15 new developer seats provisioned in the last hour, 300% above normal Tuesday activity.</p></div><button>Review Events</button></aside>
