...
function ApplicationsPage() {
  ...
  return (
    <div className="dashboard-page applications-page">
      ...
      <aside className={`sidebar ${menuOpen ? 'sidebar--open' : ''}`}><div className="sidebar-brand"><strong>PeopleFlow</strong><span>Enterprise HR</span></div><nav className="side-nav">{menu.map(([icon, label, href]) => <a className={label === 'Applications' ? 'active' : ''} href={href} key={label}><span>{icon}</span>{label}</a>)}</nav><div className="user-card"><div className="avatar">SJ</div><div><strong>Sarah Jenkins</strong><span>HR Manager</span></div></div></aside>
